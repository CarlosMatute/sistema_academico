                        ...
                        <table class="jambo_table table table-hover" id="tbl_secciones" border="1">
                            ...
                            <tbody>
                                ...
                                <tr style="font-size: small;">
                                    ...
                                    <td scope="row">{{$row->hora_inicio_formato}}</td>
                                    <td scope="row">{{$row->hora_fin_formato}}</td>
                                    <td scope="row">
                                        <button type="button" class="btn btn-warning btn-icon btn-xs btn_editar" 
                                            data-bs-toggle="modal" 
                                            data-bs-target=".modal_secciones"
                                            data-id="{{$row->id}}"
                                        ...
                                        <a href="{{url('/instituciones/')}}/{{$id_institucion}}/periodos_academicos/{{$id_periodo_academico}}/secciones/{{$row->id}}/estudiantes" type="button" class="btn btn-success btn-icon btn-xs">
...
<!-- Extra large modal -->
<!-- <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target=".modal_secciones">Extra large modal</button> -->
<div class="modal fade modal_secciones" id="modal_secciones" tabindex="-1" aria-labelledby="myExtraLargeModalLabel" aria-hidden="true">
                    ...
                    <div class="col-12 grid-margin">
                        <div class="row">
                            <div class="col-lg-2 col-md-12 col-sm-12">
                                <div class="mb-3">
                                    <label class="form-label"><strong>Sección</strong> <font class="text-danger">*</font></label>
                                    <input type="text" id="seccion" class="form-control" placeholder="Ingresa la sección" />
                                </div>
                            </div>
                            <div class="col-lg-2 col-md-12 col-sm-12">
                                <div class="mb-3">
                            ...
                            <div class="col-lg-2 col-md-12 col-sm-12">
                                <div class="mb-3" id="flatpickr-date">
                                    <label class="form-label"><strong>Hora Inicio</strong> <font class="text-danger">*</font></label>
                                    <input type="text" id="hora_inicio" class="form-control" placeholder="Ingresa la hora de inicio" data-input>
                                </div>
                            </div>
                            <div class="col-lg-2 col-md-12 col-sm-12">
                                <div class="mb-3" id="flatpickr-date">
                                    <label class="form-label"><strong>Hora Fin</strong> <font class="text-danger">*</font></label>
                                    <input type="text" id="hora_fin" class="form-control" placeholder="Ingresa la hora de finalización" data-input>
                                </div>
                            </div>
                        </div>
                        <!-- Row -->
                            ...
                            <center>
                                ...
                                <div class="col-sm-12">
                                    <div class="mb-3">
                                        ...
                                        <br>
                                        <h5><label class="form-label" id="modal_seccion"></label></h5>
                                        <br>
...
@push('custom-scripts')
  ...
  <script type="text/javascript">
    ...
    var hora_fin = null;
    var rowNumber = null; 
    var url_guardar_secciones = "{{url('/instituciones/secciones/guardar')}}"; 
    ...
    $(document).ready(function () {
        ...
        //if(accion == 1){
            // $('#modal_secciones').on('shown.bs.modal', function () {
            //     const select = $('.js-example-basic-single');
                
            //     select.val('').trigger('change'); // Reemplaza '3' por el valor que deseas seleccionar

            //     select.select2({
            //         dropdownParent: $('#modal_secciones'),
            //         width: '100%'
            //     });
            // });
        //}
        ...
        $("#modal_secciones").on("show.bs.modal", function (e) {
            ...
            const select = $('.js-example-basic-single');
            select.val(id_asignatura).trigger('change'); // Reemplaza '3' por el valor que deseas seleccionar
            select.select2({
                dropdownParent: $('#modal_secciones'),
                width: '100%'
            });
        ...
        $("#btn_guardar_asignatura").on("click", function () {
            ...
            if(seccion == null || seccion == ''){
                Toast.fire({
                    icon: 'error',
                    title: 'Valor requerido para Sección.'
                })
                return true;
            }
            ...
            if(aula == null || aula == ''){
                Toast.fire({
                    icon: 'error',
                    title: 'Valor requerido para Aula.'
                })
                return true;
            }
            ...
            if(id_asignatura == null || id_asignatura == ''){
                Toast.fire({
                    icon: 'error',
                    title: 'Valor requerido para Asignatura.'
                })
                return true;
            }
            ...
            if(hora_inicio == null || hora_inicio == ''){
                Toast.fire({
                    icon: 'error',
                    title: 'Valor requerido para Hora Inicio.'
                })
                return true;
            }
            ...
            if(hora_fin == null || hora_fin == ''){
                Toast.fire({
                    icon: 'error',
                    title: 'Valor requerido para Hora Fin.'
                })
                return true;
            }
    ...
    function guardar_secciones() {
        btn_activo = false;
        //console.log(hora_inicio);
        $.ajax({
            type: "post",
            url: url_guardar_secciones,
            data: {
                accion: accion,
            ...
            success: function (data) {
                if (data.msgError != null) {
                    ...
                    for(var i = 0; i < data.secciones.length; i++) { 
                        var row= data.secciones[i]; 
                        var nuevaFilaDT=[ 
                                     row.id,row.codigo_asignatura+' '+row.asignatura,row.nombre,row.aula,row.hora_inicio,row.hora_fin,
                                      '<button type="button" class="btn btn-warning btn-icon btn-xs btn_editar" ' +
                                        'data-bs-toggle="modal" ' +
                                        'data-bs-target=".modal_secciones" ' +
                                        'data-id="' + row.id + '" ' +
